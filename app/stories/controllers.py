...
def home(botId):
    ...
    return render_template('home.html',botId=botId,botName=bot.botName)

@stories.route('/spark/<botId>')
def spark(botId):
    print("BotId",botId)
    session.__setattr__('botid',botId)
    #intents=Story.objects.get(botId=ObjectId(botId))
    #print(intents)
    print("Inside /home session bot idf",session.__getattribute__('botid'))
    return render_template('spark.html',botId=botId)
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            for each in reader:
                ...
                list2.append(ls)
                csvStories = Story(storyName=row['storyName'],intentName=row['intentName'],apiTrigger=False,speechResponse=row['speechResponse'],parameters=parameter1,labeledSentences=list2,botId=session.__getattribute__('botid'))
                csvStories.save();
...
def create(botId):
    ...
    bot=Bot.objects.get(botId=ObjectId(botId))
    return render_template('create.html',botId=bot.botId,botName=bot.botName)
